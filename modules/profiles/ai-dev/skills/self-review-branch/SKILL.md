...
| **Comments & doc**            | Comments explain _why_ not _what_ (that's the code's job). No LLM boilerplate ("Here we iterate..."), no obsolete comments, no exposition of alternatives — that belongs in the commit message or ticket. Every comment justifies its existence. Also check robustness: the same rationale restated in several places, comments enumerating today's cases instead of the rule, references to things that get renamed underneath them (sibling constants, DB constraints, file and test names), and bare measurements or version-specific defaults that rot silently. Load the `python-dev` skill for the full criteria. |
